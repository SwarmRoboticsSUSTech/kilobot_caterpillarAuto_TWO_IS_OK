all: bootldr caterpillarAuto_TWO_IS_OK ohc ohc-big ohc-arduino-8mhz ohc-arduino-16mhz

.PHONY: docs bootldr caterpillarAuto_TWO_IS_OK ohc ohc-big ohc-arduino-8mhz ohc-arduino-16mhz
KILOLIB = build/kilolib.a
bootldr: build/bootldr.elf build/bootldr.hex build/bootldr.lss
caterpillarAuto_TWO_IS_OK: build/caterpillarAuto_TWO_IS_OK.elf build/caterpillarAuto_TWO_IS_OK.hex build/caterpillarAuto_TWO_IS_OK.lss
...
build/caterpillarAuto_TWO_IS_OK.elf: caterpillarAuto_TWO_IS_OK.c $(KILOLIB) | build
	$(CC) $(CFLAGS) -o $@ $< $(KILOLIB) 
...
program-caterpillarAuto_TWO_IS_OK: build/caterpillarAuto_TWO_IS_OK.hex build/bootldr.hex
	$(AVRUP) -p m328p $(PFLAGS) -U "flash:w:build/caterpillarAuto_TWO_IS_OK.hex:i" -U "flash:w:build/bootldr.hex"
